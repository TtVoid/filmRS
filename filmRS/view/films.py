...
def films(request,curPage):
    ...
    films = Movies.objects.all()  # 导入的Article模型
    p = Paginator(films, 18)  # 分页，3篇文章一页
    if p.num_pages <= 1:  # 如果文章不足一页
        films_list = films  # 直接返回所有文章
        data = ''  # 不需要分页按钮
    else:
        page = int(curPage)  # 获取请求的文章页码，默认为第一页
        films_list = p.page(page)  # 返回指定页码的页面
